...
def login_to_website(driver, username, password):
    ...
    try:
        ...
        handle_exception("NoSuchElementException 발생", e, driver)
    except WebDriverException as e:
        handle_exception("WebDriverException 발생", e, driver)
    except Exception as e:
        handle_exception("An unexpected error 발생", e, driver)
...
def search_report(driver):
    ...
    try:
        ...
        handle_exception("An unexpected error 발생 콘솔을 확인하세요", e, driver)
...
def open_report(driver):
    try:
        ...
        handle_exception("TimeoutException 발생", e, driver)
    except NoSuchElementException as e:
        handle_exception("NoSuchElementException 발생", e, driver)
    except WebDriverException as e:
        handle_exception("WebDriverException 발생", e, driver)
...
#    원하는 날짜를 입력하는 함수
def enter_dates(driver, date_1, date_2, date_3, date_4):
...
#    예외 처리를 수행하고 경고창을 띄우는 함수
def handle_exception(message, e, driver):
...
def process_xlsx_file(
    xlsx_file, mysql_connection_params, download_folder, xlsx_complete_folder
):
    """
...
def main():
    ...
    # 리포트 검색
    search_report(driver)

    # 리포트 열기
    open_report(driver)

    # 날짜 입력 및 다운로드
    enter_dates(driver, date_1, date_2, date_3, date_4)
