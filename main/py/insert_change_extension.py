...
def process_xlsx_file(xlsx_file, mysql_connection_params, existing_table_name):
    ...
    with MySQLConnection(**mysql_connection_params) as db:
        ...
        # 데이터 삽입
        insert_data_from_df(cursor, existing_table_name, df)
...
if __name__ == "__main__":
    ...
    existing_table_name = "3.012_receiving_tat_report"
    
    if os.path.exists(os.path.join(download_folder, xlsx_file_name)):
        process_xlsx_file(xlsx_file_name, mysql_connection_params, existing_table_name)
    else:
